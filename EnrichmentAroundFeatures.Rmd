...
lapply(names(Reduced_Variants), function(element_name) {
  ...
  speciesANDbuild <- str_split(element_name, "_MA_", simplify = TRUE)[,2] # Isolate species and build
  build <- str_split(speciesANDbuild, "_", simplify = TRUE)[,2] # Isolate just the build (is a sub-directory in species folder)
  species <- str_split(speciesANDbuild, "_", simplify = TRUE)[,1] %>% str_replace(., "^(.)(.*)$", "\\1_\\2")
  ...
  # Step Four: Intersect Variant and Gene Body Ranges and Plot Enrichment Using EnrichedHeatmap
      ...
      EnrichedHeatmap(body_variants, 
        ...
        png(filename = paste("/home/ocdm0351/DPhil/R_Data/Plots/GeneBodyOnly_", speciesANDbuild, "_Peaks.png"),
        bg = "transparent",
        type="cairo",
        units = "in",
        width = 6,
