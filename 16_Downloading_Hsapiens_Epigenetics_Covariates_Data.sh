...
#SBATCH --job-name=14_Downloading_Hsapiens_Epigenetics_Covariates_Data
...
input_file="/home/ocdm0351/DPhil/scripts/Hsapiens_Epigenetic_Data_Source_File.tsv"
...
tail -n +2 "$input_file" | while IFS=$'\t' read -r modification url start_build end_build chain_file chain_file_path; do
    ...
    wget -q -O "${outfile%.bedgraph}.bigwig" "$url" && bigWigToBedGraph "${outfile%.bedgraph}.bigwig" "$outfile" && rm -f "${outfile%.bedgraph}.bigwig"
