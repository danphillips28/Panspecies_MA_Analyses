...
```{r Summarising Variants in a Table}
...
summary_table <- imap_dfr(Annotated_VCFs_Raw, function(df, nm) {

  variant_level <- df %>%
    group_by(Variant_ID) %>%
    summarise(
      hits_protein_coding_gene_body = any(
        !EFFECT %in% excluded_effects & BIOTYPE == "protein_coding",
        na.rm = TRUE
      ),
      .groups = "drop"
    )
  ...
  tibble(
    element_name = nm,
    total_unique_variants = n_distinct(df$Variant_ID),
    unique_variants_hit_at_least_one_protein_coding_gene_body =
      sum(variant_level$hits_protein_coding_gene_body),
    unique_variants_never_hit_a_protein_coding_gene_body =
      sum(!variant_level$hits_protein_coding_gene_body)
  )
})

summary_table_subset <- summary_table %>%
  filter(str_detect(element_name, "Assaf|Saxena"))

table_df <- summary_table_subset %>%
  transmute(
    Sample = element_name,
    `Total variants` = format(total_unique_variants, big.mark = ",", scientific = FALSE),
    `Coding gene body` = format(
      unique_variants_hit_at_least_one_protein_coding_gene_body,
...
tg <- tableGrob(
  ...
  theme = ttheme_default(
    ...
    core = list(
      ...
      bg_params = list(fill = "white", col = "grey70", lwd = 0.8)
    ),
    colhead = list(
      fg_params = list(fontface = "bold", hjust = 0.5, x = 0.5),
      bg_params = list(fill = "grey95", col = "grey70", lwd = 1)
    )
  )
)

ragg::agg_png(
  filename = paste0(R_Data, "Plots/1_Summaries/Variant_Annotation_Summary.png"),
  width = 2400,
  height = 900,
  res = 300
)
...
Species_dNdS_MutationRate_Quantile_Fits <- list()
Species_dNdS_MutationRate_Quantile_Summaries <- list()
Species_dNdS_MutationRate_GAM_Fits <- list()
Species_dNdS_MutationRate_GAM_Summaries <- list()
...
ggsave(
  ...
  Species_dNdS_MutationRate_Quantile_Fits[[name]] <<- rq_fits
  ...
  Species_dNdS_MutationRate_Quantile_Summaries[[name]] <<- rq_stats
  ...
  ggsave(
    ...
    filename = paste0(
      R_Data,"Plots/3_Mutation_and_dNdS_Association_Plots/",
      name, "_dNdS_vs_Mutation_Rate_Quantile_Regression_Plot.png"
    ),
    bg = "transparent",
    width = 5.5,
    height = 4,
  ...
  Species_dNdS_MutationRate_GAM_Fits[[name]] <<- gam_fit
  Species_dNdS_MutationRate_GAM_Summaries[[name]] <<- gam_sum
  ...
  ggsave(
    ...
    filename = paste0(
      R_Data,"Plots/3_Mutation_and_dNdS_Association_Plots/",
      name, "_dNdS_vs_Mutation_Rate_GAM_Plot.png"
    ),
    bg = "transparent",
    width = 4,
    height = 4,
  ...
  if ("Essential" %in% colnames(df)) {
    ...
    if (length(unique(na.omit(ess_df$Essential))) == 2) {
      ...
      ggsave(
        plot = p1,
        filename = paste0(R_Data, "Plots/3_Mutation_and_dNdS_Association_Plots/",
                          element_name, "_Essential_Boxplot.png"),
        width = 2.5, height = 5,
  ...
  if ("HouseKeeping" %in% colnames(df)) {
    ...
    if (length(unique(na.omit(hk_df$HouseKeeping))) == 2) {
      ...
      ggsave(
        plot = p2,
        filename = paste0(R_Data, "Plots/3_Mutation_and_dNdS_Association_Plots/",
                          element_name, "_HouseKeeping_Boxplot.png"),
        width = 2.5, height = 4,
        background = "transparent"
      )
    }
    ...
    df <- df %>%
      mutate(
        ...
        Overall_Mutation_Rate_Group_More = ifelse(
          ...
          ifelse(
            is.na(UniqueVariantsPlusOnePerBP), NA,
            as.character(cut_number(log(UniqueVariantsPlusOnePerBP), n = 15, labels = FALSE))
          )
        )
      )
    return(df)
...
Annotated_VCFs_PC_Gene_Body_Variants <- lapply(
  ...
  function(df) {
    df %>%
      mutate(Overall_dNdS_Group = cut_number(dNdS + runif(length(dNdS), -1e-6, 1e-6), n = 5, labels = FALSE)) %>%
      mutate(Overall_dNdS_Group = factor(Overall_dNdS_Group, ordered = TRUE)) %>%
      mutate(Overall_dNdS_Group_More = cut_number(dNdS + runif(length(dNdS), -1e-6, 1e-6), n = 15, labels = FALSE)) %>%
